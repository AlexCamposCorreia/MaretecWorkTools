...
def copy_meteo_files(yaml, meteoModel, start, end):
    ...
    if 'meteoRemoveStartupSeconds' in yaml['getMeteoPy']['meteoModels'][meteoModel].keys():
        meteoRemoveStartupSeconds = yaml['getMeteoPy']['meteoModels'][meteoModel]['meteoRemoveStartupSeconds']
        ...
        for root, dirs, files in os.walk(yaml['getMeteoPy']['meteoModels'][meteoModel]['meteoDirectory']):
            for f in files:
                try:
                    ...
                    continue
                file_dates = [file_date_start, file_date_start + timedelta(seconds=yaml['getMeteoPy']['meteoModels'][meteoModel]['meteoFileTotalSeconds'])]
    ...
    if files_in_range == []:
        print('\n--- ERROR ---\nNo files with the specified name or date range were found in ' + yaml['getMeteoPy']['meteoModels'][meteoModel]['meteoDirectory'])
        exit(1)
